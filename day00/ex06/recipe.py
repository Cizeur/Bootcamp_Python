...
def new_recipe(cookbook: dict = {}, name: str = "",
               ingredients: list = ["love"],
               meal: str = ["anytime"], prep_time: int = 42):
    if not name:
        return
...
def add_recipe(cookbook: dict, name: str):
    ...
    if name in cookbook.keys():
        ...
        while not len(ingredients):
            ingredients = input("Enter ingredients separated with"
                                " comma[,] at least one : ").lower()
            ingredients = list(map(str.strip, ingredients.split(",")))
            while "" in ingredients:
                ingredients.remove("")
        meal_type = ""
        while not meal_type:
            meal_type = input("Enter the type of meal"
                              "[starter, lunch, dessert, etc...] : ").lower()
        while 1:
            try:
